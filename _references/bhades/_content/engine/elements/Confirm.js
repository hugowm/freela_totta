...
Confirm.prototype.draw = function(){
    ...
    this.question = new PIXI.Text(this.text, {wordWrap : true, wordWrapWidth : 360, font:'800 30px Calibri', fill:'#00779d'});
    this.question.x = this.box.x + 20;
    this.question.y = this.box.y + 20;
    this.addChild(this.question);
    
    this.btSim = new PIXI.Text('SIM', {font:'800 50px Calibri', fill:'#a6ca5c'});
    this.btSim.x = this.box.x + 50;
    this.btSim.y = this.box.y + this.box.height - 60;;
    this.addChild(this.btSim);
    
    
    this.btNao = new PIXI.Text('NÃO', {font:'800 50px Calibri', fill:'#ed1c24'});
...
Confirm.prototype.open = function(){
    ...
    TweenMax.to(this.fundo, .5, {alpha:.7, ease:Circ.easeOut, onComplete:function(){
        TweenMax.to(self.box, .5, {y:'+=350', ease:Circ.easeOut});
        TweenMax.to(self.question, .5, {y:'+=350', ease:Circ.easeOut});
        TweenMax.to(self.btNao, .5, {y:'+=350', ease:Circ.easeOut});
        TweenMax.to(self.btSim, .5, {y:'+=350', ease:Circ.easeOut});
    }});
}
